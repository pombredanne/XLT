...
import com.xceptance.xlt.report.util.pngj.ImageLineBI.Png2BufferedImageAdapter;
...
public class ImageLineSetBI implements IImageLineSet<ImageLineBI> {
    ...
    // this is the number of lines of the "target" (BI if reading PNG, PNG
    // elsewhere), less or equal than the rows of the
    // source
    private final int nlines;
    // how many lines to skip from the source
    private final int offset;
    // steps is in the source; eg. if step=2, and isForWritePng==false, we are
    // creating a BI with half the lines ofthe
    // PNG;
    // if step=2 and isForWritePng==false, we are creating a PNG with half the lines
    // of the BI
    private final int step;

    /** this constructor is for reading a PNG to a BufferedImage */
    public ImageLineSetBI(ImageInfo imgInfo, Png2BufferedImageAdapter adapter, boolean singleCursor, int nlines,
    ...
    /**
     * The row number here referers to the PNG, not the BI See
     * {@link #getImageLineRawNum(int)}
     */
     ...
     * <p>
     * Warning: this always returns a valid matrix row (clamping on 0 : nrows-1, and
     * rounding down)
     * <p>
